...
let universities = [];
let ContactInfoSorted = [];
for (let i = 0; i < graduates.length; i++){
    let university = graduates[i].university
    universities.push(university)
}
ContactInfoSorted = universities.sort()

console.log(ContactInfoSorted)
...
let unisWithUni = [];
for (let i = 0; i < graduates.length; i++){
    let uniTag = graduates[i].university;
    if(uniTag.includes('Uni')){
...
zooAnimals.forEach(function(currentValue){
  displayNames.push(currentValue.scientific_name + currentValue.animal_name)
});
...
zooAnimals.filter(function(zooFilter){
  return zooFilter.population < "5";
});
...
const populationTotal = 
  zooAnimals.reduce(function(accumulator, animalPop){
    ...
    console.log(`The Population: ${animalPop.population}`);
